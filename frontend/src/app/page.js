...
//       if (sender !== "You") {
//         setUsers((prev) => {
//           const filtered = prev.filter((u) => u !== sender);
...
//           }));
//           setNotification(`New message from ${sender}`);
...
//       ]);
//       // Move the group to the top of the joinedGroups list
//       if (sender !== username) {
//         setJoinedGroups((prev) => {
//           const filtered = prev.filter((g) => g !== groupName);
//           return [groupName, ...filtered];
//         });
//         // Increment unread count if not in the active group
//         if (groupName !== groupName) {
//           setUnreadCounts((prev) => ({
//             ...prev,
//             [groupName]: (prev[groupName] || 0) + 1,
//           }));
//           setNotification(`New message in group: ${groupName}`);
//           setTimeout(() => setNotification(""), 3000);
//         }
//       }
//     });
...
//       setJoinedGroups((prev) => {
//         if (!prev.includes(groupName)) return [groupName, ...prev]; // New group at top
//         return prev;
//       });
//       setGroupMap((prev) => ({
//         ...prev,
...
//       const chatId = mode === "private" ? receiver : groupName;
//       if (chatId) {
//         setUnreadCounts((prev) => ({
//           ...prev,
//           [chatId]: 0,
...
//             if (key === username) {
//               newCounts[data.username] = prev[key];
//             } else {
...
//       setUsers((prev) => {
//         const filtered = prev.filter((u) => u !== receiver);
...
//       setUsers((prev) => {
//         const filtered = prev.filter((u) => u !== name);
...
//       // Move the selected group to the top
//       setJoinedGroups((prev) => {
//         const filtered = prev.filter((g) => g !== name);
//         return [name, ...filtered];
//       });
//     }
//   };
...
//                           <span
//                             className={`ml-2 w-3 h-3 rounded-full ${onlineUsers.get(user) ? "bg-green-500" : "bg-gray-400"}`}
//                             data-testid={`status-${user}`}
//                           ></span>
//                         </div>
//                         <p className="text-xs text-gray-500">
//                           {onlineUsers.get(user) ? "Online" : "Offline"}
//                         </p>
//                       </div>
//                       {unreadCounts[user] > 0 && (
...
//                       {getAvatar(group)}
//                     </div>
//                     <p className="font-medium ml-3 flex-1">{group}</p>
...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    socket.on("privateMessage", ({ _id, sender, text, seenBy = [], isDeleted }) => {
      setMessages((prev) => [...prev, { _id, sender, text, seenBy: seenBy || [], isDeleted, type: "text" }]);
      // Move the sender to the top of the users list
      if (sender !== "You") {
        setUsers((prev) => {
          const filtered = prev.filter((u) => u !== sender);
    ...
    socket.on("groupMessage", ({ _id, groupName, sender, text, seenBy = [], isDeleted }) => {
      setMessages((prev) => [
        ...prev,
        { _id, sender: `[${groupName}] ${sender}`, text, seenBy: seenBy || [], isDeleted, type: "text" },
      ]);
      // Move the group to the top of the joinedGroups list
      if (sender !== username) {
        setJoinedGroups((prev) => {
    ...
    socket.on("groupNotification", ({ _id, groupName, text }) => {
      setMessages((prev) => [
        ...prev,
        { _id, sender: "System", text, groupName, type: "notification" },
      ]);
      setJoinedGroups((prev) => {
        const filtered = prev.filter((g) => g !== groupName);
        return [groupName, ...filtered];
    ...
    socket.on("messageHistory", (history) => {
      setMessages(
        history.map((msg) => ({
          ...
          seenBy: msg.seenBy || [],
          type: msg.type || "text",
          sender: msg.type === "notification" ? "System" : msg.sender,
        }))
      );
      // Reset unread count for the active chat
      const chatId = mode === "private" ? receiver : groupName;
  ...
  const getMessageStatus = (msg) => {
    if (msg.isDeleted || msg.sender !== "You" || msg.type === "notification") return null;
    if (mode === "private") {
      return msg.seenBy.includes(receiver) ? (
        <span className="text-blue-500">✔✔</span>
      ) : (
          ...
          <>
            <div className="bg-gradient-to-r from-green-600 to-teal-500 p-4 flex items-center justify-between shadow-md">
              <div className="flex items-center">
                <button
                  onClick={handleBack}
                  className="text-white hover:text-gray-200 transition mr-3"
            ...
            >
              {messages.map((msg, i) => (
                ...
                >
                  {msg.type === "notification" ? (
                    <div className="max-w-xs p-2 bg-gray-200 text-gray-600 text-sm italic rounded-lg shadow-md text-center">
                      {msg.text}
                    </div>
                  ) : (
                    <div
